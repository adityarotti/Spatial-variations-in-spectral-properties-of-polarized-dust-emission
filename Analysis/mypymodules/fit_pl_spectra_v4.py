import healpy as h
import numpy as np
import pol_binned_master_v1 as bm #Updated to version which also calculates TE spectra.
...
class fit_spectra(object):
	...
	def __init__(self,lmin,lmax,mask,globalmask,masklmax,deltaell,blpwc=np.ones(2048,float),plot_kernel=False):
		...
		self.master.setup_binning(self.deltaell,pol=True)
	...
	def calc_te_tb_spectra(self,tmap,emap,bmap,clthry=np.zeros(2048,float),maskdata=True):
		"""
			Provide filtered maps to this routine
		"""

		if maskdata:
			talm=h.map2alm(tmap*self.mask,lmax=self.lmax)
			ealm=h.map2alm(emap*self.mask,lmax=self.lmax)
			balm=h.map2alm(bmap*self.mask,lmax=self.lmax)
		else:
			talm=h.map2alm(tmap,lmax=self.lmax)
			ealm=h.map2alm(emap,lmax=self.lmax)
			balm=h.map2alm(bmap,lmax=self.lmax)

		x,clt=self.master.return_bmcs_sub_thry(h.alm2cl(talm),pol=False,TPcross=False)
		x,cle=self.master.return_bmcs_sub_thry(h.alm2cl(ealm))
		x,clb=self.master.return_bmcs_sub_thry(h.alm2cl(balm))

		# TE
		self.ubclte=self.master.return_mcs(h.alm2cl(talm,ealm),TPcross=True)
		self.lbin,self.clte=self.master.return_bmcs_sub_thry(h.alm2cl(talm,ealm),clthry,TPcross=True)
		self.clte_err=np.sqrt(2./((2.*self.lbin+1)*self.fsky*self.deltaell))*np.sqrt(clt*cle)

		# TB
		self.ubcltb=self.master.return_mcs(h.alm2cl(talm,balm),TPcross=True)
		self.lbin,self.cltb=self.master.return_bmcs(h.alm2cl(talm,balm),TPcross=True)
		self.cltb_err=np.sqrt(2./((2.*self.lbin+1)*self.fsky*self.deltaell))*np.sqrt(clt*clb)

		self.te_pow=np.sum((2.*self.ell+1)*(self.ubclte-clthry[:self.lmax+1]))/(4.*np.pi)
		self.tb_pow=np.sum((2.*self.ell+1)*self.ubcltb)/(4.*np.pi)
	...
	def fit_ee_spectra(self,map1,map2,clthry=np.zeros(2048,float),maskdata=True):
		...
		x,binclthry=self.master.return_binned_spectra(clthry)
	...
	def fit_bb_spectra(self,map1,map2,clthry=np.zeros(2048,float),maskdata=True):
		...
		self.clbb_err=np.sqrt(2./((2.*self.lbin+1)*self.fsky*self.deltaell))*np.sqrt(cl1*cl2)
        ...
        def write_spectra(self,path,ipix):
            fname=path+"cl_ee_bb_ipix" + str(ipix) + ".fits"
            d=[self.lbin,self.clee,self.clee_err,self.clbb,self.clbb_err]
            h.write_cl(fname,d,overwrite=True)


	def make_ee_bb_ps_plot(self,path="./",figname="",i353=-1000,theta=-1000,clthry=np.zeros((2,2048),float),clcmb=np.zeros((2,2048),float),plot_unbinned=False,plotlog="linear",plotfit=True,plotthry=False,plotcmb=False):
	...
	def make_te_tb_ps_plot(self,path="./",figname="",i353=-1000,theta=-1000,clthry=np.zeros((2,2048),float),clcmb=np.zeros((2,2048),float),plot_unbinned=False,plotlog="linear",plotthry=False,plotcmb=False):
		...
		if not(np.array_equal(clthry[0][:],np.zeros(2048,float))) and plotthry:
			ax1.plot(self.ell,self.ell*(self.ell+1)*clthry[0][:self.lmax+1]/(2.*np.pi),"k-",lw=1,alpha=0.5,label=r"$C_{\ell}^{\rm TE}$ dust $({\rm A}^{EE}=197.1$ ; ${\alpha}^{EE}=-2.42)$")

		if not(np.array_equal(clcmb[0][:],np.zeros(2048,float))) and plotcmb:
			ax1.plot(self.ell,self.ell*(self.ell+1)*clcmb[0][:self.lmax+1]/(2.*np.pi),"m-",lw=1,alpha=0.5,label=r"$C_{\ell}^{\rm TE}$ CMB")

	        ax1.errorbar(self.lbin,self.clte,yerr=self.clte_err,linestyle="none",marker='o',ms=3,mec="none",mfc="b",elinewidth=1,label=r"Binned $C_{\ell}^{\rm TE}$")
		if plot_unbinned:
			ax1.plot(self.ell,self.ell*(self.ell+1)*self.ubclte/(2.*np.pi),"c-",lw=1,alpha=0.3,label="")

	        ax1.hlines(0,0,max(self.lbin)+100,linestyle="dashed")

		#----------------------
		if not(np.array_equal(clthry[1][:],np.zeros(2048,float))) and plotthry:
			ax2.plot(self.ell,self.ell*(self.ell+1)*clthry[1][:self.lmax+1]/(2.*np.pi),"k-",lw=1,alpha=0.5,label=r"$C_{\ell}^{\rm TB}$ dust $({\rm A}^{BB}=104.5$ ; ${\alpha}^{BB}=-2.44)$")

		if not(np.array_equal(clcmb[1][:],np.zeros(2048,float))) and plotcmb:
			ax2.plot(self.ell,self.ell*(self.ell+1)*clcmb[1][:self.lmax+1]/(2.*np.pi),"m-",lw=1,alpha=0.5,label=r"$C_{\ell}^{\rm TB}$ CMB")

	        ax2.errorbar(self.lbin,self.cltb,yerr=self.cltb_err,linestyle="none",marker='o',ms=3,mec="none",mfc="b",elinewidth=1,label=r"Binned $C_{\ell}^{\rm TB}$")
		if plot_unbinned:
			ax2.plot(self.ell,self.ell*(self.ell+1)*self.ubcltb/(2.*np.pi),"c-",lw=1,alpha=0.3,label="")
