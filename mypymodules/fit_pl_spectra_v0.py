...
class fit_spectra(object):
	...
	def make_plot(self,path="./",figname="",i353=-1000,theta=-1000,clthry=np.zeros((2,2048),float),clcmb=np.zeros((2,2048),float),plot_unbinned=False,plotlog="linear",plotfit=True,plotthry=False,plotcmb=False):
		...
		if plotfit:
			ax1.plot(self.lbin,self.plfit(self.lbin,self.fitee[0],self.fitee[1]),"g-",lw=1,label=r"$A^{\rm EE}=$" + str(round(self.fitee[0],2)) + "$\pm$" + str(round(self.fitee[2],2)) + r"; $\alpha_E=$" + str(round(self.fitee[1],2)) + "$\pm$" + str(round(self.fitee[3],2)) )
		        ax1.plot(self.lbin,self.plfit_sc(self.lbin,self.fitee[4]),"r--",lw=1,label=r"$A^{\rm EE}=$" + str(round(self.fitee[4],2)) + "$\pm$" + str(round(self.fitee[5],2)) + r"; $\alpha^0_E=-2.42$")
		...
		#----------------------
		if plotfit:	
			ax2.plot(self.lbin,self.plfit(self.lbin,self.fitbb[0],self.fitbb[1]),"g-",lw=1,label=r"$A^{\rm BB}=$" + str(round(self.fitbb[0],2)) + "$\pm$" + str(round(self.fitbb[2],2)) + r"; $\alpha_B=$" + str(round(self.fitbb[1],2)) + "$\pm$" + str(round(self.fitee[3],2)))
		        ax2.plot(self.lbin,self.plfit_sc(self.lbin,self.fitbb[4]),"r--",lw=1,label=r"$A^{\rm BB}=$" + str(round(self.fitbb[4],2)) + "$\pm$" + str(round(self.fitbb[5],2)) + r"; $\alpha^0_B=-2.42$")
		...
		ax1.legend(loc=0,ncol=2,fontsize=6)
	        ax2.legend(loc=0,ncol=2,fontsize=6)
