...
if (whichmask.eq.0) then
  call map2alm_iterative(nside,2*nside,2*nside,2,mapin,mapalm)
...
!! -----------------------------------------------------------------
!! USES FULL SKY Q/U MAPS TO DERIVE E/B harmonics.
!! -----------------------------------------------------------------
!call map2alm_iterative(nside,2*nside,2*nside,maxiter,mapin,mapinalm)
...
!
!filename=trim(adjustl(pathout))//"rdxfs.fits"
!tempmap(:,1)=dx(:,1)
!call write_bintab(tempmap,npixtot,1,mapheader,nlheader,"!"//filename)
!
!filename=trim(adjustl(pathout))//"idxfs.fits"
!tempmap(:,1)=dx(:,2)
!call write_bintab(tempmap,npixtot,1,mapheader,nlheader,"!"//filename)
!
!
!tempmap(:,1)=2.d0*(dw(:,1)*dx(:,1) + dw(:,2)*dx(:,2))
!filename=trim(adjustl(pathout))//"rdwdx.fits"
!call write_bintab(tempmap,npixtot,1,mapheader,nlheader,"!"//filename)
!
!tempmap(:,1)=2.d0*(dw(:,1)*dx(:,2) - dw(:,2)*dx(:,1))
!filename=trim(adjustl(pathout))//"idwdx.fits"
!call write_bintab(tempmap,npixtot,1,mapheader,nlheader,"!"//filename)
!! -----------------------------------------------------------------
...
!tempmap(:,2)=mapin(:,2)*apomask(:,1)
!tempmap(:,3)=mapin(:,3)*apomask(:,1)
!call map2alm_iterative(nside,2*nside,2*nside,maxiter,tempmap,mapinalm)
...
d2w(:,1)=d2w(:,1)*apomask(:,1) !*apomask(:,1)
d2w(:,2)=d2w(:,2)*apomask(:,1) !*apomask(:,1)
!--------------------------------------------------------------------
...
!mapout(:,2)=mapout(:,2) + residual(:,1)
!mapout(:,3)=mapout(:,3) + residual(:,2)
...
tempmap(:,1)=2.d0*(dw(:,1)*dw(:,1) - dw(:,2)*dw(:,2))*mapin(:,2) - 4.d0*dw(:,1)*dw(:,2)*mapin(:,3)
filename=trim(adjustl(pathout))//"rxdwdw.fits"
call write_bintab(tempmap,npixtot,1,mapheader,nlheader,"!"//filename)

tempmap(:,1)=2.d0*(dw(:,1)*dw(:,1) - dw(:,2)*dw(:,2))*mapin(:,3) + 4.d0*dw(:,1)*dw(:,2)*mapin(:,2)
filename=trim(adjustl(pathout))//"ixdwdw.fits"
call write_bintab(tempmap,npixtot,1,mapheader,nlheader,"!"//filename)

tempmap(:,1)=2.d0*(dw(:,1)*dx(:,1) + dw(:,2)*dx(:,2))
filename=trim(adjustl(pathout))//"rdwdwx.fits"
call write_bintab(tempmap,npixtot,1,mapheader,nlheader,"!"//filename)

tempmap(:,1)=2.d0*(dw(:,1)*dx(:,2) - dw(:,2)*dx(:,1))
filename=trim(adjustl(pathout))//"idwdwx.fits"
call write_bintab(tempmap,npixtot,1,mapheader,nlheader,"!"//filename)
